function posner_recode_v3_DevMIND_wo_fixation
...
%                  12297 or 12298
%
%                  Same as the "posner_recode_v3" code with target-response
%                  pairs being changed.
%		  
%                  
...
DefaultName = 'Posner_Recode_Behavior_Data_wo_fixation';
...
for i = 1:iter                        %loop throught participants                  
    ...
    for ii = 3:(length(triggers)-2)
        ...
        if (triggers(ii) == 12297 | triggers(ii) == 12298) && (triggers(ii+1) == 12289 | triggers(ii+1) ==  12291) %left valid cue followed by target%
            ...
            if (triggers(ii+1) == 12289 & triggers(ii+2) == 12800) | (triggers(ii+1) == 12291 & triggers(ii+2) == 12544)
        ...
        elseif (triggers(ii) == 12297 | triggers(ii) == 12298) && (triggers(ii+1) == 12294 | triggers(ii+1) ==  12296) %right valid cue followed by target%
            ...
            if (triggers(ii+1) == 12294 & triggers(ii+2) == 12800) | (triggers(ii+1) == 12296 & triggers(ii+2) == 12544)
        ...
        elseif (triggers(ii) == 12297 | triggers(ii) == 12298) && (triggers(ii+1) == 12293 | triggers(ii+1) ==  12295) %right valid cue followed by target%
            ...
            if (triggers(ii+1) == 12293 & triggers(ii+2) == 12800) | (triggers(ii+1) == 12295 & triggers(ii+2) == 12544)
        ...
        elseif (triggers(ii) == 12297 | triggers(ii) == 12298) && (triggers(ii+1) == 12290 | triggers(ii+1) ==  12292) %right valid cue followed by target%
            invalid_trial_counter = invalid_trial_counter+1;
            if (triggers(ii+1) == 12290 & triggers(ii+2) == 12800) | (triggers(ii+1) == 12292 & triggers(ii+2) == 12544)
...
%Loop to print the behavioral data to an excel sheet
   ...
   files(1,1:end-4),...
   size(RT,2),...
   behavior.group_ACC(1,i),behavior.group_valid_ACC(1,i),behavior.group_invalid_ACC(1,i),...
   behavior.group_RT(1,i),behavior.group_valid_RT(1,i),behavior.group_invalid_RT(1,i),...
   behavior.group_RT_OE(1,i),behavior.group_valid_RT_OE(1,i),behavior.group_invalid_RT_OE(1,i));        %Fields applied to the output file
   else
   fprintf(outputfile, '%s\t %8.0f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\n',...
   files{i}(1,1:end-4),...
   size(RT,2),...
   ...
   behavior.group_RT_OE(1,i),behavior.group_valid_RT_OE(1,i),behavior.group_invalid_RT_OE(1,i));        %Fields applied to the output file                
        ...
        if isa(files,'char')
            filename = strcat(files(1,1:end-4),'_recoded_wo_fixation.evt');
        else
            filename = strcat(files{i}(1,1:end-4),'_recoded_wo_fixation.evt');                              %save evt file%
        end
