...
function VWM_behavior_general_recode
...
DefaultName = 'VWM_Recode_Behavior_Data';
...
fprintf(outputfile, 'ParID\t group_ACC\t group_RT\t group_correct_RT\t group_incorrect_RT\t group_RT_OE\t group_correct_RT_OE\t group_incorrect_RT_OE\t group_correct_trial_count\t group_incorrect_trial_count\t group_dprime\n');        %Headers applied to the output file
...
%     
    if size(RT,2) > 1 && size(trial_index,2) > 1
       ...
       if (sum(trial_index_dprime==1)==0)|(sum(trial_index_dprime==2)==0)|(sum(trial_index_dprime==3)==0)|(sum(trial_index_dprime==4)==0)
           behavior.group_dprime(1,i) = norminv(((sum(trial_index_dprime==1)+.5)/((sum(trial_index_dprime==1)+.5)+(sum(trial_index_dprime==3)+.5))))-norminv((((sum(trial_index_dprime==4)+.5))/((sum(trial_index_dprime==4)+.5)+(sum(trial_index_dprime==2)+.5)))); 
       else
...
%Loop to print the behavioral data to an excel sheet


   if isa(files,'char')
   fprintf(outputfile, '%s\t %8.0f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\n',...
   files(1,1:end-4),...
   behavior.group_ACC(1,i),...
   behavior.group_RT(1,i),behavior.group_correct_RT(1,i),behavior.group_incorrect_RT(1,i),...
   behavior.group_RT_OE(1,i),behavior.group_correct_RT_OE(1,i),behavior.group_incorrect_RT_OE(1,i),...
   behavior.group_correct_trial_count(1,i),behavior.group_incorrect_trial_count(1,i),behavior.group_dprime(1,i));        %Fields applied to the output file
   else
   fprintf(outputfile, '%s\t %8.0f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\t %8.4f\n',...
   files{i}(1,1:end-4),...
   behavior.group_ACC(1,i),...
   ...
   behavior.group_correct_trial_count(1,i),behavior.group_incorrect_trial_count(1,i),behavior.group_dprime(1,i));        %Fields applied to the output file
       ...
       evt_info = [time,data(1:size(time,1),2),triggers];
        if isa(files,'char')
           filename = strcat(files(1,1:end-4),'_recoded_test.evt');
        else
           filename = strcat(files{i}(1,1:end-4),'_recoded_test.evt');                              %save evt file%
        end
