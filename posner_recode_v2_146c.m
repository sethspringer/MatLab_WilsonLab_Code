...
for i = 1:iter                                                                              %loop for files%
    ...
    for ii = 3:length(triggers)
        if (triggers(ii) == 12297 | triggers(ii) == 12298) && (triggers(ii+1) == 12289 | triggers(ii+1) == 12294 | triggers(ii+1) ==  12291 | triggers(ii+1) == 12296) %valid cue followed by target%
            ...
            if ((triggers(ii+1) == 12289 | triggers(ii+1) == 12294) && (triggers(ii+2) > 12544 && triggers(ii+2) < 12551)) | ((triggers(ii+1) == 12291 | triggers(ii+1) == 12296) && (triggers(ii+2) > 12802 && triggers(ii+2) < 12809)) %correct valid target%
                triggers(ii) = 15;
                triggers(ii+1) = 25;
                triggers(ii+2) = 35;
        ...
        elseif (triggers(ii) == 12297 | triggers(ii) == 12298) && (triggers(ii+1) == 12290 | triggers(ii+1) == 12293 | triggers(ii+1) ==  12292 | triggers(ii+1) == 12295) %invalid cue followed by target%
            ...
            if ((triggers(ii+1) == 12290 | triggers(ii+1) == 12293) && (triggers(ii+2) > 12545 && triggers(ii+2) < 12550)) | ((triggers(ii+1) == 12292 | triggers(ii+1) == 12295) && (triggers(ii+2) > 12803 && triggers(ii+2) < 12808)) %correct invalid target%                if triggers(ii+1) == 12291 | triggers(ii+1) == 12296 %correct valid trial%
                triggers(ii) = 16;
                triggers(ii+1) = 26;
                triggers(ii+2) = 36;
    ...
    if size(RT,2) > 1 && size(validity_index,2) > 1
        ...
        if isa(files,'char')
            filename = strcat(files(1,1:end-4),'_recoded_NEW.evt');
        else
            filename = strcat(files{i}(1,1:end-4),'_recoded_NEW.evt');                              %save evt file%
        end
