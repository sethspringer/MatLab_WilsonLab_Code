function seth_sourcespace_corr_permutation_testing_only_suprathreshold
...
if df ~=76
    error('I manually entered the critical r value for 80 files, this needs to be updated if the number of files changes')
else
    r_critical_value = .2227
end
...
for i = 1:n_clusters
    ...
    cluster_rs = r_map(sig_masking_map_cluster_building == cluster_labels(i)); %Extract the test stats for each cluster and subtract the critical value
    
    if sum(cluster_rs) > 0 %positive r values in this cluster
        cluster_rs_subtracted = cluster_rs-r_critical_value;
    else %negative r values in this cluster
        cluster_rs_subtracted = abs(cluster_rs)-r_critical_value;
        cluster_rs_subtracted = cluster_rs_subtracted*-1;
    end
    
    cluster_sum(i) = sum(cluster_rs_subtracted); %sum to get the cluster "mass"
...
for permutation_index = 1:n_permutations
    ...
    n_clusters_perm = length(cluster_labels_perm);
    
    
    cluster_sum_perm = zeros(n_clusters_perm,1);
    
    if n_clusters_perm > 0
        clear i
        for i = 1:n_clusters_perm
            
            cluster_rs_perm = r_map(sig_masking_map_perm_cluster_building == cluster_labels_perm(i)); %Extract the test stats for each cluster
            
            %and subtract the critical value at each voxel
            if sum(cluster_rs_perm) > 0 %positive r values in this cluster
                cluster_rs_perm_subtracted = cluster_rs_perm-r_critical_value;
            else %negative r values in this cluster
                cluster_rs_perm_subtracted = abs(cluster_rs_perm)-r_critical_value;
                cluster_rs_perm_subtracted = cluster_rs_perm_subtracted*-1;
            end
            
            cluster_sum_perm(i) = sum(cluster_rs_perm_subtracted); %sum to get the cluster "mass"
